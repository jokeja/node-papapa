...
const 补仓计算 = (本次单价, 期望卖出价格, 补仓股数, 现有股数, 现有每股成本, 卖出股数) => {
  ...
  return { 补仓后每股单价, 补仓后股数, 做T利润, 上涨比例, [`卖出${卖出股数}股`]: { 卖出部分盈亏, 做T成功后成本 } }
}
// cool18.beginPapapa()
// cool18.rewrite()
// console.group('中国银行0')
...
let stocks = [
  {
    ...
    name: '渤海轮渡', Ts: [
      { 本次单价: 8.33, 期望卖出价格: 8.22, 补仓股数: 300, 现有股数: 0, 卖出股数: 0, 现有每股成本: 0 },
      { 本次单价: 8.17, 期望卖出价格: 8.22, 补仓股数: 300, 现有股数: 300, 卖出股数: 300, 现有每股成本: 0 }]
  },
  {
    name: '新能股份', Ts: [{ 本次单价: 6.56, 期望卖出价格: 6.96, 补仓股数: 1000, 现有股数: 0, 卖出股数: 0, 现有每股成本: 0 }]
  },
  {
...
stocks.forEach((stockItem) => {
  ...
  stockItem.Ts.forEach((item, index) => {
    console.group(`${stockItem.name}${item.本次单价}`)
    let result = null
    if (last补仓结果) {
